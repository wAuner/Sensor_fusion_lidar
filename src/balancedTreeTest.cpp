...
void
render2DTree(Node<pcl::PointXYZ>* node, pcl::visualization::PCLVisualizer::Ptr& viewer, Box window, int& iteration, uint depth = 0) {
    ...
    if (node != NULL) {
        ...
        // split on x axis
        if (depth % 2 == 0) {
            viewer->addLine(pcl::PointXYZ(node->point->data[0], window.y_min, 0),
                            pcl::PointXYZ(node->point->data[0], window.y_max, 0), 0, 0, 1,
            ...
            viewer->addLine(pcl::PointXYZ(window.x_min, node->point->data[1], 0),
                            pcl::PointXYZ(window.x_max, node->point->data[1], 0), 1, 0, 0,
                            "line" + std::to_string(iteration));
...
void findClusterPoints(const std::vector<std::vector<float>>& points, int pointIdx, std::vector<int>& cluster,
                       std::unordered_set<int>& processedPoints, KdTree<pcl::PointXYZ>* tree, float distanceTol) {
